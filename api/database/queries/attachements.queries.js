...
const saveAttachment = async (emailId, filename, contentType, content) => {
...
const getAttachmentsByEmailId = async (emailId) => {
    const query = "SELECT id, filename, content_type FROM attachments WHERE email_id = $1";
...
const getAttachmentByEmailIdAndId = async (emailId, id) => {
    const query = "SELECT id, filename, content_type, content FROM attachments WHERE email_id = $1 AND id = $2";
